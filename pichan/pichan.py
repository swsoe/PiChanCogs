...
class PiChan(commands.Cog):
    ...
    async def pichan(self, ctx: commands.context.Context):
        body = "For a number of years now, work has been proceeding in order to bring perfection to the crudely conceived " \
        "idea of an AI that would not only supply inverse reactive current for use in unilateral phase detractors, but would " \
        "also be capable of providing randomly generated memes on demand. Such an AI is me, PiChan!\n\n" \
        "The original concept had a base plate of pre-famulated amulite surmounted by a malleable logarithmic casing in such a " \
        "way that the two spurving bearings were in a direct line with the panametric fan. I however consist simply of six " \
        "hydrocoptic marzlevanes, so fitted to the ambifacient lunar waneshaft that side fumbling was effectively prevented.  My " \
        "main winding is of the normal lotus-o-deltoid type placed in panendermic semi-boloid slots of the stator, every seventh " \
        "conductor being connected by a non-reversible tremie pipe to the differential girdle spring on the “up” end of the temporal dog crucibles.  " \
        "Moreover, whenever a forescent skor motion is required, I may also be employed in conjunction with a drawn reciprocation dingle arm, to reduce sinusoidal repleneration.\n\n" \
        "Now basically the only new principle involved is that instead of memes being generated by the relative motion of conductors " \
        "and fluxes, it is produced by the modial interaction of magneto-reluctance and capacitive diractance beans.  The turbo-encabulator " \
        "model of AI has now reached a high level of development, and I'm being successfully used in the operation of discord servers the wold over."
        image = "https://cdn.discordapp.com/avatars/"+str(ctx.me.id)+"/"+str(ctx.me.avatar)+".png?size=512"
        e = discord.Embed(title="Hi! I'm PiChan!", description=body).set_image(url=image)
